    ...
    <div class="max-w-3xl mx-auto px-4 py-12">
      <div class="flex items-center justify-between mb-8">
        <div>
          ...
          <p class="text-gray-500 text-sm mt-1">Keys for the MCP gateway to load workflows.</p>
        </div>
        <button
          @click="generateKey"
          :disabled="generating"
      ...
      <div v-if="newKey" class="mb-8 bg-green-50 border border-green-200 rounded-lg p-4">
        ...
        <div class="flex items-center gap-2">
          <code class="flex-1 bg-[#0f0f13] border border-green-300 rounded px-3 py-2 text-sm font-mono text-green-800 select-all">{{ newKey }}</code>
          <button @click="copyKey" class="px-3 py-2 bg-green-600 hover:bg-green-700 text-gray-100 rounded text-sm transition-colors">
            {{ copied ? 'Copied' : 'Copy' }}
          </button>
        </div>
      </div>
      ...
      <div v-else-if="keys.length === 0 && !newKey" class="text-center py-12">
        ...
        <p class="text-gray-500 text-sm">Generate a key to connect agents to the gateway.</p>
      </div>
      ...
      <div v-else class="space-y-3">
        <div
          v-for="key in keys"
          :key="key.id"
          class="bg-[#131318] border border-[#2e2e35] rounded-lg px-5 py-4 flex items-center justify-between"
        >
          <div>
            <span class="font-mono text-sm text-gray-500">{{ key.prefix }}...</span>
            <span v-if="key.name" class="text-sm text-gray-500 ml-3">{{ key.name }}</span>
          </div>
          <div class="flex items-center gap-4">
            <span v-if="key.last_used" class="text-xs text-gray-500">Last used {{ formatDate(key.last_used) }}</span>
...
export default {
  setup() {
    ...
    async function fetchKeys() {
      loading.value = true
      try {
        const records = await api.getKeys()
        // The gateway stores only the hashed key; display its first 7 chars as the prefix.
        keys.value = records.map(k => ({
          ...k,
          prefix: (k.hashed_key || '').slice(0, 7),
    ...
    async function generateKey() {
      ...
      copied.value = false
      try {
        // The gateway generates and returns the raw key once.
